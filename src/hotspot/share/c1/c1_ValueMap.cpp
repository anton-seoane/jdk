...
#include "utilities/bitMap.inline.hpp"
  ...
  #define TRACE_VALUE_NUMBERING(code) if (ul_enabled(Compilation::current(), Debug, jit, valuenumbering)) { code }
...
void ValueMap::increase_table_size() {
  ...
  TRACE_VALUE_NUMBERING(log_debug(jit, valuenumbering)("increasing table size from %d to %d", old_size, new_size);)
...
Value ValueMap::find_insert(Value x) {
  ...
  if (hash != 0) {
    ...
    for (ValueMapEntry* entry = entry_at(entry_index(hash, size())); entry != nullptr; entry = entry->next()) {
      if (entry->hash() == hash) {
        ...
        if (!is_killed(f) && f->is_equal(x)) {
          ...
          TRACE_VALUE_NUMBERING(log_debug(jit, valuenumbering)("Value Numbering: %s %c%d equal to %c%d  (size %d, entries %d, nesting-diff %d)",
                                                               x->name(), x->type()->tchar(), x->id(), f->type()->tchar(), f->id(), size(),
                                                               entry_count(), nesting() - entry->nesting());)
    ...
    TRACE_VALUE_NUMBERING(log_debug(jit, valuenumbering)("Value Numbering: insert %s %c%d  (size %d, entries %d, nesting %d)",
                                                         x->name(), x->type()->tchar(), x->id(), size(), entry_count(), nesting());)
  }
      ...
      if (must_kill) {                                                                   \
        ...
        if (prev_entry == nullptr) {                                                     \
          ...
          prev_entry = entry;                                                            \
                                                                                         ...
                                                                                         \
        TRACE_VALUE_NUMBERING(log_debug(jit, valuenumbering)("Value Numbering: killed %s %c%d  (size %d, entries %d, nesting-diff %d)", value->name(), value->type()->tchar(), value->id(), size(), entry_count(), nesting() - entry->nesting());)   \
      } else {                                                                           \
        prev_entry = entry;                                                              \
      }                                                                                  \
    }                                                                                    \
...
LoopInvariantCodeMotion::LoopInvariantCodeMotion(ShortLoopOptimizer *slo, GlobalValueNumbering* gvn, BlockBegin* loop_header, BlockList* loop_blocks)
  ...
  TRACE_VALUE_NUMBERING(log_debug(jit, valuenumbering)("using loop invariant code motion loop_header = %d", loop_header->block_id());)
  TRACE_VALUE_NUMBERING(log_debug(jit, valuenumbering)("** loop invariant code motion for short loop B%d", loop_header->block_id());)
...
void LoopInvariantCodeMotion::process_block(BlockBegin* block) {
  TRACE_VALUE_NUMBERING(log_debug(jit, valuenumbering)("processing block B%d", block->block_id());)
  ...
  while (cur != nullptr) {
    ...
    if (cur_invariant && is_dominated_by_inputs(_insertion_point, cur)) {
      ...
      TRACE_VALUE_NUMBERING(LogMessage(jit, valuenumbering) msg;
        NonInterleavingLogStream st(LogLevelType::Debug, msg);
        st.print_cr("Instruction %c%d is loop invariant", cur->type()->tchar(), cur->id());
        cur->print_line(&st);
      )
...
bool ShortLoopOptimizer::process(BlockBegin* loop_header) {
  TRACE_VALUE_NUMBERING(log_debug(jit, valuenumbering)("** loop header block");)
  ...
  for (int i = 0; i < _loop_blocks.length(); i++) {
    ...
    TRACE_VALUE_NUMBERING(log_debug(jit, valuenumbering)("processing loop block B%d", block->block_id());)
  ...
  TRACE_VALUE_NUMBERING(log_debug(jit, valuenumbering)("** loop successfully optimized");)
  return true;
}
...
{
  TRACE_VALUE_NUMBERING(log_debug(jit, valuenumbering)("****** start of global value numbering");)
  ...
  for (int i = 1; i < num_blocks; i++) {
    ...
    TRACE_VALUE_NUMBERING(LogMessage(jit, valuenumbering) msg;
      NonInterleavingLogStream st(LogLevelType::Debug, msg);
      st.print_cr("value map before processing block: ");
...
void GlobalValueNumbering::substitute(Instruction* instr) {
  ...
  if (subst != instr) {
    ...
    TRACE_VALUE_NUMBERING(log_debug(jit, valuenumbering)("substitution for %c%d set to %c%d", instr->type()->tchar(), instr->id(), subst->type()->tchar(), subst->id());)
