...
#ifndef PRODUCT
  if (log_is_enabled(Trace, eliminateallocations) && safepoints != nullptr) { //PEA
...
#ifndef PRODUCT
      if (log_is_enabled(Trace, eliminateallocations)) { //PEA
        LogMessage(eliminateallocations) msg;
        NonInterleavingLogStream st(LogLevelType::Trace, msg);
        if (field != nullptr) {
...
#ifndef PRODUCT
  if (alloc->is_AllocateArray()) //PEA
    log_trace(eliminateallocations)("++++ Eliminated: %d AllocateArray", alloc->_idx);
  else
    log_trace(eliminateallocations)("++++ Eliminated: %d Allocate", alloc->_idx);
#endif
